...
% no input bounds, with OBSTACLES. min of u and slack norm
...
%             minimize( norm( u(1:2) , 2) + norm(slack,2) + dh_dxj*robot(2).g*u2 )    
            minimize(  norm(slack(1),2) - dh_dxj*robot(2).g*u2 )  
...
%             minimize( 10*norm(slack,2))  
            ...
            for i=1:n_robots
                for j=1:n_obstacles
                    ...
                    if (j==1)
                        robot(i).obs_1_dh_dx(data_counter,:) = -dh_dxi;
                        robot(i).obs_1_h(data_counter) = h;
                        robot(i).obs_1_upper_bound(data_counter) = ( robot(i).obs_1_h(1) - h )/alpha_cbf;
                        robot(i).obs_1_sum_h(data_counter+1) = robot(i).obs_1_sum_h(data_counter) + h*dt;
                    else
                        robot(i).obs_2_dh_dx(data_counter,:) = -dh_dxi;
                        robot(i).obs_2_h(data_counter) = h;
                        robot(i).obs_2_upper_bound(data_counter) = ( robot(i).obs_2_h(1) - h )/alpha_cbf;
                        robot(i).obs_2_sum_h(data_counter+1) = robot(i).obs_2_sum_h(data_counter) + h*dt;
                    end
...
%                 u_ref(2*i-1:2*i,1) = nominal_controller(robot(i),u_min,u_max);         
            ...
            robot(1).obs_1_inner_prod(data_counter) = robot(1).obs_1_dh_dx(data_counter,:) * robot(1).dV_dx(data_counter,:)';
            robot(1).obs_2_inner_prod(data_counter) = robot(1).obs_2_dh_dx(data_counter,:) * robot(1).dV_dx(data_counter,:)';
...
title('Obstacle 1')
hold on
plot(tplot,robot(1).obs_1_inner_prod,'r')
plot(tplot,-robot(1).obs_1_h,'k')
plot(tplot,robot(1).V,'m')
yline(0)
legend('Robot 1',' Obs barrier','1 Lyapunov')

figure(6)
title('Obstacle 2')
hold on
plot(tplot,robot(1).obs_2_inner_prod,'r')
plot(tplot,-robot(1).obs_2_h,'k')
plot(tplot,robot(1).V,'m')
yline(0)
legend('Robot 1','Obs barrier','1 Lyapunov')
...
figure(7)
title('Obstacle 1')
plot(tplot,robot(1).obs_1_sum_h(2:end))
hold on
plot(tplot,robot(1).obs_1_upper_bound,'k')
legend('\int h  dt','upper_bound')

figure(8)
title('Obstacle 2')
plot(tplot,robot(1).obs_2_sum_h(2:end))
hold on
plot(tplot,robot(1).obs_2_upper_bound,'k')
legend('\int h  dt','upper_bound')
