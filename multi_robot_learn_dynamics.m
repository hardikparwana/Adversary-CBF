...
gp_normal(1) = MatrixVariateGaussianProcess(Omega,sigma,l);
gp_normal(2) = MatrixVariateGaussianProcess(Omega,sigma,l);
gp_normal(3) = MatrixVariateGaussianProcess(Omega,sigma,l);
   ...
   cvx_begin quiet
    ...
    for i=1:n_robots        
        ...
        elseif (data_counter<=3)
            ...
            %predict
            robot(i).predicted_data(:,end+1) = gp(i).predict(robot(i).input_data(:,end)')';
            robot(i).predicted_normal_data(:,end+1) = gp_normal(i).predict(robot(i).input_data(:,end)')';
        end
