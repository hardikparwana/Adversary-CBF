...
% Robots 
n_robots = 2;
robot(1) = Unicycle2D(1,0,2,pi,1,2,'nominal');   %ID,x,y,yaw,r_safe,D,status
robot(2) = Unicycle2D(2,1,6.5,0,1,2,'nominal'); 
   ...
   cvx_begin quiet
       ...
       subject to 
            ...
            % Interagent collisions
                    ...
                    dh_dxj*( robot(2).f + robot(2).g*u2 ) > 0;%3; % -alpha_cbf*h;
...
%             minimize( norm( u(1:2) , 2) + norm(slack,2) + dh_dxj*robot(2).g*u2 )   
            obj = norm(slack(1),2) - dh_dxj*robot(2).g*u2;
...
%             minimize( 10*norm(slack,2))  
            ...
            robot(2).G = [robot(1).X(1); robot(1).X(2)];
            u_ref(3:4,1) = nominal_controller(robot(2),u_min,u_max);   
