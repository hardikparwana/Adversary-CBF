...
for t=0:dt:tf
    ...
    subject to       
        ...
        for i=1:1:num_robots
           ...
           A = dh_dxj; b = -robot(i).human_alpha * h - dh_dxi*robot(i).inputs(:,end); rho_dist = A*u_human - b - 0.2
           rho_dist = tanh(rho_dist);
           theta_as = real(acos( A*u_human/norm(A)/norm(u_human) ));
           theta_ns = real(acos( A*u_nominal(:,i)/norm(A)/norm(u_nominal(:,i)) )); 
           if (theta_ns<0.05)
               theta_ns = 0.05;
           end
           rho_theta = -1 + tanh(theta_ns/theta_as);           
           ...
           A = dh_dxj; b = -robot(i).robot_alpha(1) * h - dh_dxi*robot(i).inputs(:,end); rho_dist = A*robot(index).inputs(:,end) - b - 0.2
           rho_dist = tanh(rho_dist);
